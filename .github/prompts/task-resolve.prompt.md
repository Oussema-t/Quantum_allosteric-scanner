...
- After successful resolution, auto-chain to `workflow.task.promote` when all criteria pass:
	- resolved task is on-disk `.ai/tasks/TASK-*.md`
	- resolution is `done` or `fixed`
	- user did not request `no-promote`
- For auto-chain promotion, extract at minimum:
	- UI intent baseline-like sections to `.ai/tasks/contracts/`
	- architecture-relevant findings to `.ai/reviews/`
- Auto-chain should be additive and retrieval-first: update existing matching artifacts when present instead of creating duplicates.
- If promotion confidence is low or source sections are missing, skip auto-chain and report why.
...
- `Promotion:` include only when auto-chain promotion executed; report promoted sections and target artifact paths.
