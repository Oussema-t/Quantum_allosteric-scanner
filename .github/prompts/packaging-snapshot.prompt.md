...
- current provider entrypoint: [capability-runner.sh](../../agents-tools/capability-runner.sh) with `repo.packaging.snapshot`
- optional follow-up capability: `repo.packaging.branch-file-list`

Required behavior:

- Treat this as the `repo.packaging.snapshot` capability.
- Use the capability runner entrypoint unless a narrower equivalent capability call is clearly better.
