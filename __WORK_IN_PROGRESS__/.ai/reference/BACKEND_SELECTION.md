...
| Test Execution | local repository execution via repo-aware tools | active default | Project-specific runners such as Testkube are future backend adapters until the execution capability names them explicitly. |
...
- Remote execution backends such as Testkube should not be treated as active defaults until the execution capability supports them directly.
