...
from task0309_kmeans_extended_cohort import lrt  # [[TASK-0319]]: this file's own
# local _ll/lrt (two independent samples drawn per null replicate -- a
# difference of log-likelihoods on UNRELATED datasets, not a likelihood
# ratio) is retired in favour of task0309's generalised, TASK-0319-corrected
# implementation, per this task's own Scope ("one implementation, not two").
...
def part_ab(S):
    ...
    print("=" * 70)
    # [[TASK-0319]]: the ORIGINAL controls below (run once at B=300x5, using
    # this file's own local `lrt` -- since retired, see the top-of-file
    # import) were generated by a two-independent-samples bug: the k=2 and
    # k=1 null fits each drew a FRESH `r.normal(mu, sd, n)` sample, so the
    # "LRT" was a difference of log-likelihoods on unrelated datasets, not a
    # likelihood ratio -- it roughly doubled the null's variance and made
    # the test over-conservative in a way that still looked power-like
    # (positive controls reached p<0.05 by sep=1.5-2.0 SD). Recomputed here
    # at n=28, B=300, 5 reps, with the corrected `task0309.lrt` (single
    # sample per null replicate, non-degenerate resampling): power at this
    # n is far WORSE than the old table suggested -- essentially none up to
    # 3.0 SD separation. This is a materially different, more honest
    # picture, not a re-confirmation of the old one.
    ctrl = {"pos_sep1.0": [0.910, 0.355, 0.824, 0.176, 0.455],
            "pos_sep1.5": [0.302, 0.405, 0.405, 0.960, 0.203],
            "pos_sep2.0": [0.781, 0.821, 0.734, 0.458, 0.767],
            "pos_sep3.0": [0.392, 0.252, 0.100, 0.166, 0.043],
            "neg": [0.266, 0.864, 0.581, 0.156, 0.751]}
    for k, v in ctrl.items():
        print(f"  {k:<12} p = " + ", ".join(f"{x:.3f}" for x in v))
    o, p = lrt(np.log(y), 1, 2, B=400, seed=1, seeds=3)
    ...
    print(f"\n  *** VERDICT: negative control clean (0/5 false positives at n=28),")
    print(f"  *** but positive controls show the corrected test has essentially NO")
    print(f"  *** power at this n even at 3.0 SD separation (1/5 reps significant).")
    print(f"  *** p={p:.3f} is INCONCLUSIVE for lack of power, not evidence either way.")
    out['lrt'] = dict(LR=o, p=p, separation_sd=float(sep), controls=ctrl,
                      verdict="uninformative -- negative control clean but the test has "
                              "no demonstrated power at this n even at 3.0 SD; corrects "
                              "the prior 'underpowered above 2.0 SD' characterisation, "
                              "which used the two-sample-bug controls")
    return out
