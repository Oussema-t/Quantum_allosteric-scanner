...
**Our honest position: single-particle coherent transport on a static contact graph is exhausted.** We closed nine candidate advantage routes by measurement, then tested the natural tenth directly rather than merely proposing it: a finite-delay, phase-sensitive amplitude `2·Re⟨r|e^{-iHτ}|a⟩`, the one construction that provably retains what the converged limit above discards. Scored under a principled per-protein clock (τ fixed by the Hamiltonian's own spectral gap, no sweep), it is nominally significant at the pre-registered tolerance (cluster-permutation p = 0.036) and not significant at any tolerance one order of magnitude to either side (p = 0.46–0.96) — a τ-selection artifact, not a signal, and even a clean positive here would be a better observable rather than a demonstrated quantum advantage (a 2×2 permanent is exactly as easy as a 2×2 determinant). A remaining quantum route must supply something the static graph does not have — either a true many-body object (conformational search, whose hard regime we have located at 50–80 residues) or dynamics rather than one structure.
