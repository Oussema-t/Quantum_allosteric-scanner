...
**We then closed the question directly.** The converged propagator we use is *provably phase-free* — a result we derived rather than inherited — so we built three observables to reach past it (chiral circulation, frequency-domain coherence, two-boson interference) and finally measured the thing itself: the **same** Hamiltonian, seed, cohort and scoring, run coherently and decoherently. Across 108 structures in 76 protein clusters the difference is **+0.0023 AUC, Wilcoxon p = 0.92, cluster-robust p = 0.83** — a well-powered null, not an underpowered one.
...
**Our honest position: the phase-free construction is exhausted.** We closed nine candidate advantage routes by measurement, then built the natural tenth rather than merely proposing it: a finite-delay, phase-sensitive amplitude `2·Re⟨r|e^{-iHτ}|a⟩`, the one construction that provably retains what the converged limit above discards. Evaluated at a delay set by the Hamiltonian's own spectral gap it carries no information at all — AUC 0.503, correlation with proximity −0.016 — which is the expected reading rather than a surprise, since that clock is defined as the time by which transients have died and the converged limit is provably phase-free. Its unsigned magnitude does clear chance at one tolerance (cluster-permutation p = 0.036) and not at neighbouring ones across a delay range spanning a factor of three. **The short-delay regime, where the interference term is still alive, we have not yet measured.** It is the one quantum route we carry into Phase 2, with its ceiling stated: even a clean positive there would be a better observable, not a demonstrated advantage (a 2×2 permanent is exactly as easy as a 2×2 determinant). Any route beyond it must supply something the static graph does not have — either a true many-body object (conformational search, whose hard regime we have located at 50–80 residues) or dynamics rather than one structure.
