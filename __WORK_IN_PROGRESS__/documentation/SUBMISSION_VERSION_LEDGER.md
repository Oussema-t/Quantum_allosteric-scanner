...
| **§ Prior art: ref [8] sentence sharpened**, stating precisely what it does (VQE structure prediction for already-located, orthosteric fragments) and precisely what it doesn't (detection, apo state, allosteric sites) | [[TASK-0364]] — read the paper (not just its abstract) before shipping a sentence about the sponsor's own group's work; the original sentence was correct but only defensible against the abstract, not the method/evaluation sections | Keep the specific delta clauses. A vaguer "no claim this domain is untouched" reopens exactly the ambiguity this read closed |
