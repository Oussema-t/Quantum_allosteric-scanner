...
|---|---|---|---|
| 10 (TASK-0070, TASK-0052, TASK-0047, TASK-0058, TASK-0063, TASK-0055, TASK-0064, TASK-0071, TASK-0056, TASK-0067) | 1 (TASK-0079 — .001/.002/.003/.004 Done, .005 TODO) | 24 | 35 |
...
| # | Task | Why | Status |
|---|---|---|---|
| 5.1 | **[FILED] TASK-0079 — end-to-end challenge run** (split into **.001–.005** subtasks 2026-07-12) | Produce the three **required deliverables** for every mandatory target (KRAS 4OBE→6OIM, BCR-ABL1 1OPL→5MO4, CARDIAC_MYOSIN; MYC_MAX deferred to TASK-0080): the **N×N connectivity matrix**, the **top-5 ranked hit list**, and the **methodological report**. Currently no single command produces them. This is the submission artifact. | **In Progress** — .001/.002/.003 **Done** (see prior rows); .004 (`scripts/run_challenge.py --target NAME [--dry-run]`, one command wiring `.002`+`.003` end-to-end, writes `connectivity_matrix.npz`/`hit_list.json`/`report.txt`/`verdict.json` per target) **Done** — 10 new tests against a synthetic/mocked target (control-flow + file-writing, no network), 460 passed. Found (not fixed, filed as **TASK-0090**) a real `select.py` crash on a multi-index seed source — worked around with a single representative seed index in the orchestrator. Holo-side numbers (`AUC_holo_*`, `mean_rho_apo_holo`, `mean_jacc20`) deliberately left unwired (real cross-frame mapping work of its own, out of scope). .005 (live run against KRAS_G12C/BCR_ABL1/CARDIAC_MYOSIN) TODO, now unblocked |
