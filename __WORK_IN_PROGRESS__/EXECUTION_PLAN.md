...
| 1.3 | **[EXISTS] TASK-0064** — wire unsupervised score into frozen loop | Completes the label-free selection path. | **Done.** `protocol.select_frozen_config(build_candidates, held_out_target)` added -- takes a *callable*, not a pre-built candidate list, so both construction and scoring run inside `frozen_context`, catching a leaky candidate-builder, not just a leaky scoring call. Lives in `protocol.py` itself, alongside the other gated accessors. `SEAM-0009` closed to VERIFIED (2 seam-tests). `INV-0004` seeded for `select.py`'s reported quantities, `TASK-0089` filed as its owner. 405 passed (was 402). **Update 2026-07-20 ([[TASK-0089]]): `INV-0004` closed** -- every seeded row classified (GAUGE-VERIFIED/KNOB-CHARACTERIZED/SIGNAL null-controlled), 35 new tests. Real finding: `source_specificity` is not relabeling-invariant with its default sub-sampled `n_alt` (root-caused to label-position-dependent `rng.choice`, confirmed exact once sampling is exhaustive) -- reclassified GAUGE->KNOB rather than force-passed or silently "fixed" into a different behavior; flagged in the function's own docstring. `unsupervised_score`'s ranking also confirmed to flip between scalar/multi-index `source` conventions (KNOB, matches TASK-0118's project-wide finding). Full detail: `.ai/invariants/INV-0004-select-unsupervised-score.md`. |
...
| 1C.21 | **[FILED] TASK-0139 — resolve KRAS_G12C's contradictory learnability verdicts** | TASK-0120's whole-structure CO said `LEARNABLE`; TASK-0133's pocket-restricted CO disagreed (below threshold, but only p≈0.07 elevated over a random-patch null) and left it "reported, not decided" — a live contradiction under this project's most-cited positive reframe, not one more honest-negative confirmation. | **Done (2026-07-20).** Decided both open questions from first principles, not by picking the convenient answer: (1) pocket-restricted CO is the correct quantity (the RMSD half of the same conjunction is already region-specific by design; a whole-structure CO answers a different, easier question than the panel's own kill criterion asks); (2) `co_threshold=0.5` was never validated against any null (confirmed directly — its only other consumer, `cumulative_overlap_gate`, doesn't validate it either) and is replaced by a direct significance test against TASK-0133's own random-patch null where one exists. New `superpose.learnability_verdict(co_percentile=..., significance_alpha=0.05)` (ADD-only, ambiguous default byte-identical to every prior call) adds a real third verdict, `AMBIGUOUS`, for CO evidence that clears neither tail. Applied programmatically to the real, already-computed TASK-0120/0133 numbers (`scripts/resolve_kras_learnability.py`, no new expensive computation): **KRAS_G12C resolves to `AMBIGUOUS`** (RMSD ratio 2.27 clears its bar; restricted CO's 93rd-percentile elevation over the null does not clear α=0.05 in either direction). BCR_ABL1/CARDIAC_MYOSIN confirmed unchanged (`LEARNABLE`, RMSD-determined, CO half never reached either way). Propagated additively into `RESULTS.md`'s learnability-gate section and open-questions rows 12/17. Full detail: `RESULTS.md`'s learnability-gate section, `results/tasks/0139_learnability_resolution/resolution.json`. |
| 1C.22 | **[FILED] TASK-0136 — path-ensemble + percolation connectivity between active site and pocket** | Raised directly by the orchestrating user, 2026-07-18: "would it make sense to actually make a shortest path finding algorithm... and do any form of percolation on the outcome?" Purely topological (no propagator, no clock, no seed coherence) -- deliberately outside the P0 gauge-fixing batch's confound axes. | **Done (2026-07-22).** New `allostery.percolation` (shortest-path ensemble via Dijkstra+Yen's algorithm, Menger's-theorem edge connectivity + literal min-cut, Union-Find percolation-threshold sweep) + `baselines.connectivity_robustness` (part c, a new blind scoreable baseline, edge-connectivity seed-to-every-residue). **Headline: all 3 mandatory targets show broad, redundant connectivity (68-76 edge-disjoint routes) between active site and pocket -- no narrow bottleneck anywhere.** Real bug found+fixed before trusting any number: the virtual super-source/sink construction used unit-capacity edges, silently capping every reported connectivity at `min(|active|,|pocket|)` (confirmed via a synthetic single-hub-3-fan-out check that true connectivity isn't bounded by set cardinality) -- fixed via large-capacity virtual edges + `nx.maximum_flow_value`/`minimum_cut` (`nx.edge_connectivity`/`minimum_edge_cut` don't accept a `capacity` argument at all, checked directly). Pre-fix numbers (13-18, empty cuts) were wrong; post-fix (68-76, real multi-edge cuts) are correct. KRAS_G12C's percolation threshold merges after just 2 edges (3.75Å) -- an independent topological confirmation of this project's own established active-site/pocket proximity finding. Part (c) scored: beats floor on 1/3 targets (KRAS_G12C only); TASK-0123's `stratified_auc` cross-read actually run (not cited by analogy) -- confirms most of the raw AUC is the same distance/degree confound found pervasive elsewhere in this project's register (stratified mean AUC 0.372-0.535, near or below chance). Dumbbell gate (new `TestConnectivityRobustnessDumbbellGate`): exactly 0.500 in every cell -- a clean, understood negative (unweighted route-*counting* can't see the dumbbell's edge-*weight*-only confound), not ambiguous. Full detail: `.ai/tasks/DONE/TASK-0136-percolation-connectivity-robustness.md`. |
| 1C.23 | **[FILED] TASK-0134 — CPU-time re-verification of the `time_averaged_ctqw` infeasibility claim, plus a reusable long-job convention** | [[P-0005]], raised directly by the orchestrating user, 2026-07-18: TASK-0110's "did not return after 2+ hours" claim was corroborated by process state (`R`, active) at kill time, not by continuous CPU-time evidence -- a process starved by contention (already proven real in this sandbox, TASK-0111) or suspended/resumed would look identical at one checkpoint. | **Done (2026-07-22).** Part 1: re-ran the same call (real KRAS_G12C, `H_new`, AAKV-prescribed `t_max`/`n_steps`) with continuous `allostery.runlog.RunLogger` instrumentation, 4-thread BLAS cap, 438 samples over one full, uninterrupted 950s/15.8min run to completion. **`cpu_elapsed_s/wall_elapsed_s` held at 4.05 +/- 0.035 throughout, no drops anywhere -- corroborated, not inflated.** Real complication found live: the same formula on the same target now prescribes `t_max=1.26e6`/`n_steps=877,811`, ~15x smaller than TASK-0110's original `4.82e6`/`1.3e7` -- `H_new`'s spectrum shifted after TASK-0121's renormalization (landed the day after TASK-0110's measurement); extrapolated to the original scale the re-verified rate implies ~3.9 CPU-hours, consistent with "still computing after 2+ hours." Separate methodological finding: an uncapped calibration run measured CPU-time at ~16x wall-clock (matching the 16-core machine, multithreaded BLAS, not contention) -- the real contention diagnostic is ratio *stability* across samples, not magnitude, easy to get backwards on a first pass. Part 2: new `.ai/reference/LONG_JOB_CONVENTION.md` (cross-linked from `IMPLEMENTER_SPINUP_BRIEF.md`/`OPERATION_PROTOCOL.md`) -- the ~10-15min synchronous-blocking threshold, harness-tracked vs. OS-level detachment for two distinct time horizons, monitor-by-tail not poll, and an explicit HITL hand-off section. Downstream citations (`INV-0005`, `SEAM-0012`, `P-0005`) updated additively with the corroboration + parameter-drift finding. TASK-0105's ENAQT per-gamma timings and TASK-0068's NISQ Trotter-step timing flagged (not re-run) as sharing the same unverified-CPU-time shape. |
...
| 1D.2 | **[FILED] [[TASK-0140]] — chiral (broken-time-reversal) circulation observable (HYP-P9)** | **P1.** The disciplined form of the collaborator's Idea #2 (an arbitrary initial phase is a label-leaking knob; complex Peierls hopping is the only gauge-invariant phase, acting on cycles — Zimborás 2013, Lu 2016). Hodge-decomposed circulating current is proximity-orthogonal by construction; passed both synthetic gates this batch (dissociates coupling from well; beats floor at ρ≈+0.33 vs CTQW's +0.6..+0.97). Single-particle → modeling advantage, NOT asymptotic. Soft-gated by 1D.1. Eval with CIs ([[TASK-0112]]) + stratified AUC ([[TASK-0123]]) + permutation null ([[TASK-0131]]). | **Done (2026-07-23).** Reference script confirmed absent; reconstructed independently, new `allostery.chiral` (35 tests). Both synthetic gates re-verified passing on this fresh build (own numbers, not the lost script's 0.33/0.6-0.97 — GATE 1 loop-dumbbell mean AUC 0.81/0.19 in the two conflict cells; GATE 2 AUC 0.93-0.98 vs. occupation not beating the floor). Real finding along the way: a bare N-cycle ring gives exactly zero converged circulation at any flux (ruled out the obvious minimal test fixture, not a real-data concern). **Real run, all 7 targets: FAIL** — CI overlap `True` everywhere, the primary pre-registered bar not met on any target, consistent with 1D.1's own FAIL. KRAS_G12C/PTP1B clear the Bonferroni-corrected stratified-AUC permutation null alone (suggestive, not a PASS). Proximity-orthogonality itself confirmed cleanly on 7/7 targets. **1D.4's own gate (needs 1D.1 OR 1D.2 to show life) remains unmet.** Full detail: `RESULTS.md`'s own chiral-circulation section, open-questions row 28; `.ai/tasks/DONE/TASK-0140-chiral-circulation-observable.md`. |
...
| 1D.4 | **[FILED] [[TASK-0142]] — Hodge-L1 + persistent H2 observable (HYP-P12)** | **P2, gated on 1D.1/1D.2 showing life** for the L1 half only — **H2 half ungated 2026-07-22** (Architect/Planner correction: a capped void's lining is graph-*adjacent*, the opposite of TASK-0143's open-cleft graph-*far* premise, demonstrated directly). The correct topological home for the multi-site closure: a capped pocket is an H2 *void* (not H1 loop — why Falsifier D's grooves were invisible); the ligand-as-k-simplex is what L1 keeps and the graph Laplacian discards. The one rung where the biology (capped cavity) meets a genuine quantum-advantage object (quantum Betti / Hodge-Laplacian estimation, LGZ; Jones-polynomial BQP-completeness). Real persistence tooling (GUDHI/Ripser) mandatory — no homemade b1 as evidence. Matched random-patch null ([[TASK-0133]]). | **H2 half Done (2026-07-24) — FAIL, clean and one case sharply informative.** 2/3 mandatory targets show no H2 void at all (top persistence at/below the synthetic noise floor); the third (CARDIAC_MYOSIN) detects a void, but the wrong one (AUC 0.192, matched-null percentile 0.0 — anti-ranks the real pocket). Proximity floor beats `void_score` on real data (0.583 vs 0.192), confirming the synthetic control's own residue-localization weak link. **L1 half: TODO, still gated, not run** — neither leg (1D.1/TASK-0143, 1D.2/TASK-0140) is open; both are FAIL. Full detail: `RESULTS.md`'s own persistent-H2 section, open-questions row 30; `.ai/tasks/DONE/TASK-0142-hodge-l1-persistent-h2.md`. |
| 1D.5 | **[FILED] [[TASK-0145]] — quantum transport / effective-conductance observable (Landauer-Buttiker + classical R_eff)** | **P1.** Reframes the scoring question from "where does a seeded excitation spread to" to "what is the steady-state current/transmission from the active site to each candidate residue" — the formalism used in molecular electronics (Landauer-Buttiker; Nitzan & Ratner), not a seeded-walk-and-wait one. Complements, does not duplicate, [[TASK-0136]]'s discrete percolation/edge-connectivity work — a continuous conductance/transmission measure. | **Done (2026-07-24).** New `allostery.transport`: classical `R_eff` (Moore-Penrose pseudo-inverse of a genuine combinatorial Laplacian — `H_new` structurally does not qualify, confirmed directly) + Landauer-Buttiker `T(E)` (wide-band-limit leads, `E=0` DC-limit primary). Real algorithmic contribution: a Sherman-Morrison closed form gets every candidate's `T(E)` from one `N x N` inversion instead of `N` separate ones (`O(N^2)` not `O(N^4)`), verified against brute force to `1e-10`. Dumbbell gate: clean double dissociation against GSR on both observables (C2/C3 exactly 1.000/0.000); `R_eff` provably well-invariant (confirmed, not assumed). **Real-target headline: 1/9 cells (3 targets x {R_eff, T(E) on L, T(E) on H_new}) Bonferroni-significant** — BCR_ABL1's `T(E=0)` on the bare Laplacian (p=0.003, AUC 0.699); 2 more `H_new`-based cells clear the floor at point estimate only (uncorrected-significant). Classical-vs-quantum (point 3 of the Intent Contract): Spearman rho 0.74-0.85 (strongly correlated, confirms the `E=0` choice) but not interchangeable — `T(E=0)` decisively beats `R_eff` on BCR_ABL1's identical graph. `E` is a large, real KNOB (>2x, non-monotonic AUC swing on BCR_ABL1's own grid), characterized not exploited. Full detail: `RESULTS.md`'s own transport section, open-questions row 31. |
| 1D.6 | **[FILED] [[TASK-0149]] — low-mode PRS/DCC predictor (`prs_low`/`dcc_low`, HYP-adjacent, proximity-orthogonal by construction)** | **P1.** Delivered pre-built and pre-tested on synthetic data only (`.ai/reviews/2026-07-22/`, RCSB unreachable from the delivering thread's own sandbox, honestly disclosed). Restricts ANM pseudo-inverse response (`prs_low`) / GNM covariance (`dcc_low`) to the `k` lowest modes — the subspace the learnability gate ([[TASK-0120]]/[[TASK-0133]]) already showed carries the real apo→holo displacement, scored here as a *predictor* for the first time. | **Done (2026-07-24).** Ported unmodified; the 3 delivered tests re-verified passing. Re-ran on real targets under [[TASK-0118]]'s current seed convention, gated through [[TASK-0123]]'s stratified-AUC + permutation-null machinery, swept `k_modes` in {5,10,15,20}. **KRAS_G12C/BCR_ABL1: genuinely dead** (p=0.67-0.99 at every k, matching the delivering thread's own predicted possible outcome). **CARDIAC_MYOSIN: real, Bonferroni-surviving signal** (stratified well-powered max AUC 0.92/`prs_low`, 0.96/`dcc_low` at k=20, p=0.006/p<0.001, survives the primary 6-comparison Bonferroni bar). **Real divergence from the synthetic control's own prediction, not smoothed over**: ρ(score,-hop) stays substantial on every real target (`prs_low` -0.18 to -0.51 anti-proximity; `dcc_low` +0.44 to +0.63, the classic proximity-confound direction) — far from the synthetic control's ~0.08-0.16 "cleanly decorrelated" result — so CARDIAC_MYOSIN's whole-graph AUC alone is not trustworthy proof of signal; the stratified/permutation-null result is the actual finding. A real sign bug found+fixed in this task's own new evaluation script (not the ported predictor code). Cross-referenced, not conflated, with [[TASK-0122]]'s `CP_low`. Full detail: `RESULTS.md`'s own low-mode-PRS/DCC section, open-questions row 30. |
| 1D.7 | **[FILED] [[TASK-0146]] — frequency-domain / spectral coherence observable (un-averaged coherent oscillation)** | **P2.** Every propagator this project scores explicitly time-averages away phase information (`time_averaged_ctqw_converged` is provably phase-free at the converged limit). Tests whether the frequency content of the un-averaged coherent amplitude `c_j(t)=<j|exp(-iHt)|source>`, Fourier-analyzed, carries coupling information the converged limit destroys. Architect/Planner's own prior: lower plausibility than 1D.5/TASK-0145 — no structural argument for proximity-orthogonality, just a different observable on the same underlying dynamics. | **Done (2026-07-24).** New `allostery.spectral_coherence` — score = total AC (non-DC) spectral power of `p_j(t)=|c_j(t)|^2` (equals `Var_t[p_j(t)]` exactly by Parseval, confirmed as a test-suite identity), `T=5000` calibrated against real Bohr-gap statistics after confirming the task's own literal "resolve the smallest gap" criterion is TASK-0110's own already-documented infeasibility. Sensitivity check: the shipped `t_max=15` default is materially inadequate (flips KRAS_G12C's own floor-clearing verdict vs. the calibrated default, which itself agrees closely with a 10x-larger window). Dumbbell gate: clean double dissociation against GSR (C2=1.000/C3=0.000 exact). **Real-target verdict: no decisive result, with a confirmed mechanism, not just an absence of signal** — all 3 targets' CIs overlap the floor, no well-powered stratified max clears the 3-target Bonferroni bar, and ρ(score,−hop)=+0.68 to +0.72 on every target — the Architect/Planner's own prior (no proximity-orthogonality guarantee) confirmed empirically. Full detail: `RESULTS.md`'s own spectral-coherence section, open-questions row 36. |
| 1D.8 | **[FILED] [[TASK-0148]] — single-particle entanglement entropy across a spatial cut (Peschel correlation-matrix method)** | **P2.** Per-candidate localization/coupling observable distinct from occupation, distance-stratified AUC, or anything else in the register — the entanglement entropy of the active site's coherent state across a spatial bipartition (a candidate's own hop-neighborhood vs. everything else), condensed-matter's own standard localization diagnostic. Re-scoped from an originally-proposed multi-body "quantum mutual information" framing to this cheaper, better-grounded single-particle form. | **Done (2026-07-24).** New `allostery.entanglement`. Citation verified (Peschel 2003, arXiv:cond-mat/0212631). **Real mathematical reduction, verified to `1e-16` before touching real data**: for a single coherent source, entropy exactly reduces to the binary Shannon entropy of the region's own total occupation probability — but real active sites are multi-residue under TASK-0118's own incoherent-mixture GAUGE, where the correlation matrix is generically rank>1 and the closed form does not apply, so real scoring used the general Peschel diagonalization throughout. **A second real finding resolving this task's own Open Question**: the converged/time-averaged occupation limit this project defaults to elsewhere is not *available* for this observable at all (needs coherent phase information `|amplitude|^2` discards) — used a fixed `t*=1/gap` coherent snapshot instead. Dumbbell gate: clean double dissociation against GSR (C2/C3 exactly 1.000/0.000). **Real-target headline: a clean, complete negative** — no target clears the proximity floor (KRAS_G12C/BCR_ABL1/CARDIAC_MYOSIN all `NO_SIGNAL_IN_APO`), no permutation-null p-value close to significant even uncorrected. Cross-read against TASK-0106: the coherent snapshot is 1.2-2.3x more localized than the converged limit on all 3 targets (consistent with the established Anderson-localization finding), but that localization does not translate into per-candidate discriminative signal. Full detail: `RESULTS.md`'s own entanglement-entropy section, open-questions row 37. |
...
| 5.4 | **[EXISTS] TASK-0015** — holo-direction module | LRT / PRS / two-state ANM / NMFF, gated by cumulative overlap. | **Done (2026-07-28).** New `holo_direction.py` (Steps 0-2: frozen perturbation protocol, admissible-deformation-family generator, go/no-go gate). Ran Step 2's own mandatory "First action" on all 7 pocket-scoreable verified targets before touching Steps 3-5, per the spec's own instruction. **Headline: 7/7 `NO_GO`** — no admissible apo-only perturbation ever creates the missing active-site<->pocket edge, no target's CO(10) clears the 0.5 threshold. Steps 3-5 (transport on predicted graphs) not built for any target, per the spec's own "build the rest only for targets that clear it." Full detail: `RESULTS.md`'s own "Holo-direction module Step 2 go/no-go gate" section, open-questions row 47. |
...
- **TASK-0117 — ceiling search shares every headline AUC's unvalidated CTQW parameters.**
  ...
  **TASK-0110 landed Done 2026-07-16-17** and directly informs TASK-0117 without closing
  ...
  `results/tasks/0110/<target>/scan.json`.
