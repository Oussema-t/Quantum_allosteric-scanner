...
|---|---|---|---|
| 10 (TASK-0070, TASK-0052, TASK-0047, TASK-0058, TASK-0063, TASK-0055, TASK-0064, TASK-0071, TASK-0056, TASK-0067) | 1 (TASK-0079 — .001-.004 Done, .005 in progress: KRAS_G12C/BCR_ABL1 done, CARDIAC_MYOSIN running) | 24 | 35 |
...
| # | Task | Why | Status |
|---|---|---|---|
| 5.1 | **[FILED] TASK-0079 — end-to-end challenge run** (split into **.001–.005** subtasks 2026-07-12) | Produce the three **required deliverables** for every mandatory target (KRAS 4OBE→6OIM, BCR-ABL1 1OPL→5MO4, CARDIAC_MYOSIN; MYC_MAX deferred to TASK-0080): the **N×N connectivity matrix**, the **top-5 ranked hit list**, and the **methodological report**. Currently no single command produces them. This is the submission artifact. | **In Progress** — .001-.004 **Done** (see prior rows; .004 found+filed **TASK-0090**, a real `select.py` multi-index-source crash, worked around not fixed inline). **.005 (live run) 2/3 done** — KRAS_G12C and BCR_ABL1 ran for real against live RCSB data; CARDIAC_MYOSIN still computing (apo N=950 residues, `targets.yaml`'s own flagged large/low-quality cryo-EM structure — expensive, confirmed not hung). Both completed targets passed every structural Acceptance Scenario check (matrix shape/symmetry, hit-list exclusion, real frozen stamp, no DEV banner) and surfaced **three genuine findings**, each spawning its own investigation task rather than being fixed inline — full reasoning in TASK-0079.005's own Done section (load-bearing, read before picking any of these up): **KRAS_G12C's real AUC (0.779) falls well outside this repo's own previously-asserted near-chance band (0.3-0.7)** for the same target — a real, unresolved methodology-sensitivity discrepancy (cutoff 8.0 vs 10.0 Å / assembled vs raw pocket label / apo- vs holo-derived source), filed as **TASK-0093**. **BCR_ABL1's classical heat kernel (AUC 0.731) sharply outperforms CTQW (0.525) on the identical winning operator** — not yet explained by this repo's existing "coherence adds ~nothing" finding (that was CTQW-vs-dephased-CTQW on KRAS, a different comparison than CTQW-vs-heat on BCR_ABL1) — filed as **TASK-0091** (calibrated dephasing sweep, reusing the KRAS precedent's exact recipe). **Holo-side numbers are `N/A` in both reports** (`.004`'s deliberate scope cut) — filed as **TASK-0092** to wire them in as a diagnostic upper bound (not a prediction substitute), extending TASK-0067's own apo-vs-holo finding (tiny gap for the bare operator) to the full `H_new` pipeline. |
