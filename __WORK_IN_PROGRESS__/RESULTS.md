...
[[TASK-0290]] found (and correctly left alone, out of its own LANE 1
scope) 6 failing tests in `backend/test_analysis_characterization.py`:
golden values pinned 2026-07-12 against KRAS_G12C's apo `4OBE`, stale
since [[TASK-0270]] (2026-08-26) corrected the config to `4LDJ` (4OBE is
wild-type, not the G12C mutant) without regenerating these fixtures. This
task regenerates them from the live code path, per its own Constraint —
no hand-edited numbers.

**Regenerated by actually calling `gnm_context`/`site_potentials`/
`quantum_seed_readiness`/`seed_readiness_shift`/`connectivity_change`
against `SYSTEMS["KRAS_G12C"]` (already `apo=4LDJ`, unchanged by this
task) and reading the live output** — not derived from the old numbers,
not guessed. All 6 tests, plus the full `backend/` suite (37/37), pass.

**A real finding survived the regeneration, not just numbers moving**:
`quantum_seed_readiness`'s own **verdict** flips **PARTIAL → RISKY**
(`n_good` 8→7, `frac_good` 0.36→0.32) — the qualitative classification
changed, not only its inputs. Consistent with the same direction every
other re-scored KRAS_G12C quantity has moved since TASK-0270 (e.g.
[[TASK-0283]]'s `NO_FAILURE_DETECTED`→`NO_SIGNAL_IN_APO`), not an
isolated anomaly in this one function — reported as a finding in the test
file itself (a code comment on the assertion), per this task's own
Constraint, not silently absorbed.

**A genuine cross-check, not just "the numbers changed"**: `seed_
readiness_shift`'s `holo`-side dict is **byte-identical** to the original
2026-07-12 (4OBE-era) capture — correct, since `holo=6OIM` never changed,
only `apo`. Confirming the untouched half of a paired computation matches
exactly while the touched half moves is stronger evidence the
regeneration is right than either fact alone.

**Checked for other silent 4OBE dependents, not assumed clean**:
`backend/test_analysis.py::test_kras_g12c_real_target_site_potentials_
and_connectivity_change_pin` also pins against `"4OBE"` — but as a
**literal, hardcoded ID** (not via `SYSTEMS`), an arbitrary-but-fixed
refactor-regression target from the same 2026-07-12 capture, never
claiming to represent "the current KRAS_G12C benchmark." It is
self-consistent (fetches and pins the same fixed ID) and was never broken
by TASK-0270's config change — confirmed still passing, not stale in the
same sense, left alone. Flagged because its numbers now happen to
duplicate the *old* (pre-TASK-0296) values in the file above it, which
could read as linked to a future reader; it is not.

**Provenance recorded in the test file itself**, per this task's own
second Scope item: a new docstring block states which structure
(`4LDJ`), which task regenerated the goldens (this one), and when
(2026-08-30) — so the next structure swap makes staleness detectable by
reading the file, not by someone else's unrelated `pytest` run catching
it by accident again.

**Incident, disclosed**: this section (and the matching test-file edit)
was lost once mid-task — another concurrent thread's `git reset` (visible
in `git reflog` as `reset: moving to HEAD`) discarded every uncommitted
tracked-file change across this shared working tree, not just this
task's own. Untracked files (this task's own `DONE` file) survived;
tracked-file edits (this section, the test file) did not and were
redone identically from the same already-computed live values — not
re-derived from memory of the numbers, the regeneration commands were
re-run. Not this task's own doing; flagged for visibility, not silently
absorbed.
