...
**KRAS G12C (`4LDJ`).** We verified the structure before using it: residue 12 is CYS, the only heteroatoms are GDP and Mg, and six apo G12C depositions exist in the PDB — Table 1's `4OBE` is wild-type at residue 12 and the organisers' suggested `8S8C` is drug-bound, so neither can serve. The result is then decided before the walk runs. Candidate pockets are cut to a top-K before seeding: under the narrower setting (detector ranking, top-5 by druggability) the true sotorasib pocket ranks **sixth** and is discarded, leaving 1 drug residue among 36 seeds and P@5 zero by construction, whatever the operator does. Widening to a druggability-and-allostery consensus at top-10 seeds it: **8 drug residues among 44**, base rate 0.182. Same structure, same walk, same scoring — the selection stage alone moved the target from unanswerable to answerable. Scored on that seed set, the best of 14 operators x 17 scores reaches AUC **0.809** and the worst **0.184**, a spread of 0.63 across cells differing only in operator and score; read correctly the worst is a reversal rather than a failure, since 1 - 0.184 = 0.816 is within noise of the best. Under the single cell fixed before scoring, the target returns an honest negative against a label-permutation null.
...
**BCR-ABL1 (`1OPL`).** The same pipeline on the myristoyl target. Here the true asciminib pocket **is** seeded (pocket 2 of the consensus set), and 18 of 50 seeds are drug-pocket residues — a base rate of 0.360, which is itself the finding rather than a convenience: 1OPL carries myristate in that pocket, as section 1 records, so the cavity is already open and the task is not cryptic-pocket prediction. Best of the same 238 cells: AUC **0.755** with P@5 0.8 (`H6_exp`, proximity-corrected residual with the focus term); worst **0.181** (`H5_gauss`, the coherence ratio), again a reversal rather than a failure since 1 - 0.181 = 0.819. The pre-registered single cell gives AUC **0.408** against a label-permutation null, p = 0.86 — below chance and indistinguishable from it.

Read together, the two targets make one point and it is not about which operator wins. On KRAS the selection stage decided the answer was unreachable; on BCR-ABL1 the selection stage delivered a pocket that was already open. In neither case did the transport subroutine determine the outcome, and in both the spread between best and worst cell (0.63 and 0.57 AUC) is larger than any margin between methods reported in section 2.
