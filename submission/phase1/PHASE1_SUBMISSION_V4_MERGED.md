...
![Connectivity C_ij for all four targets (KRAS G12C, BCR-ABL1, cardiac myosin, HIV-1 RT): best operator (top row), worst operator (middle), and their difference (bottom; red = best transports more). The first required deliverable. Best and worst share structure and seeding, so the difference is the operator-and-score choice alone; the near-uniform backgrounds show the converged walk has largely equilibrated. Full matrices and the top-5 x active-site sub-blocks are in `results/connectivity/`.](connectivity_bestworstdiff.png)
