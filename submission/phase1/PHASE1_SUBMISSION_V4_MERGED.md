...
![Connectivity matrices C_ij (best operator per target): the time-averaged CTQW transport between every residue pair, for KRAS G12C, BCR-ABL1, cardiac myosin and HIV-1 RT. Bright diagonal and near-uniform off-diagonal background show the converged walk has largely equilibrated -- the first required deliverable, one matrix per operator, in `results/connectivity/`.](connectivity_all.png)
