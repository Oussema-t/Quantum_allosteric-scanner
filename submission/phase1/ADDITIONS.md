...
![Connectivity C_ij for two representative targets (KRAS G12C, BCR-ABL1) -- best operator, worst operator, and their difference (red = best transports more). Best and worst share structure and seeding, so the difference is the operator-and-score choice alone. The first required deliverable; all four targets' matrices, and the top-5 x active-site sub-blocks, are in `results/connectivity/`.](connectivity_bestworstdiff.png)
