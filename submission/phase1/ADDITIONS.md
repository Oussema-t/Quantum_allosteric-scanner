...
**B.2 Per-target connectivity.** Best and worst cell per target -- the three mandated targets plus HIV-1 RT (NNRTI pocket) -- each selected across all 221 cells (13 Hamiltonians x 17 scores) by P@5, seeded by the consensus of fpocket druggability and PASSer allostery. Seed-pocket cut and distal filter are per-target choices, counted in the selection ceiling: top-5 pockets by min-rank (KRAS, BCR-ABL1), top-10 by rank fusion (HIV-1 RT) or by PASSer allostery alone (cardiac myosin); MIN_HOP 1 for KRAS and cardiac myosin, 2 for BCR-ABL1 and HIV-1 RT. Connectivity matrices and top-5 x active-site sub-blocks are in `results/connectivity/`. Per-target detail (connectivity sheet: best/worst matrices, difference, top-5 x active-site; walk-trace sheet: P(active,t) per seed and the winning score ranked over the seeds, so its AUC/P@5 equal the table's) is under `submission/phase1/supplementary/` in the repository.

| target (apo) | cell | operator / score | AUC | P@5 |
|---|---|---|---|---|
| KRAS G12C (`4LDJ`) [14] | best | `H9_bfac` / Green E=median | 0.806 | 0.8 |
| KRAS G12C (`4LDJ`) [14] | worst | `H11_aniso` / residual RAW | 0.184 | 0.0 |
| BCR-ABL1 (`1OPL`) [1-3] | best | `H6_exp` / residual LOG+dX | 0.796 | 1.0 |
| BCR-ABL1 (`1OPL`) [1-3] | worst | `H5_gauss` / R=p_peak/p_avg | 0.227 | 0.0 |
...
| HIV-1 RT (`1DLO`->`3V81`, NNRTI) | worst | `H9_bfac` / Green E=zero | 0.230 | 0.0 |

*Table B.2 -- best and worst of 221 cells (13 Hamiltonians x 17 scores) per target, chosen by P@5 (AUC breaks ties), each target from one run (its walk traces and matrices are the supplementary files). Cardiac myosin's and HIV-1 RT's pockets are reached only once the consensus seeds them (their true pocket ranks low on fpocket druggability but high on PASSer allostery). Best and worst share the protein and seeding, so the AUC spread (0.62/0.57/0.50/0.73) is the operator-and-score choice alone. These are selection ceilings over 221 cells, not blind performance; the pre-registered `H_new` cell is a near-negative on all four (on KRAS it is the best cell by AUC, 0.809, but only P@5 0.6). With the per-target seeding fixed every number reproduces exactly (fpocket output is cached); changing the seed-pocket cut moves the winning cell (top-10 seeding: BCR-ABL1 P@5 0.8), which is why those choices are declared part of the ceiling.*
