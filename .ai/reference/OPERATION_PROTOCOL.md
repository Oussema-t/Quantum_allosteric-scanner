...
- For local execution, prefer one stable capability command per step over ad hoc shell composition.
- Avoid chained execution one-liners that mix variable assignment, path construction, and test invocation.
- Keep execution and artifact-capture as separate explicit steps when both are needed.
- Prefer request-file mode when command arguments become long, repeated, or policy-sensitive.