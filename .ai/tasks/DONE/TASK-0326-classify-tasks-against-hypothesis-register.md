...
## Progress (2026-09-04, Implementer A) — full sweep complete, disposition drafted, landing not yet applied

**Full sweep run**: 6 parallel Explore agents (~51 tasks each), same
scheme/register cheat-sheet/verification discipline as the pilot.
Combined with the pilot: **344/344 non-meta tasks classified**
(`ENG` 204/59%, `HYP-<id>` 58/17%, `NO-HYP` 82/24% across ~45 claim
clusters, `UNCLASSIFIED` 0). Full ledger:
`.claude/hypotheses/TASK_CLASSIFICATION_LEDGER.md`.

**`NO-HYP` share roughly doubled vs. the pilot (15%→24%)** — not
recalibration, the corpus is chronologically ordered and the back half
(~TASK-0200–0325) is a sustained dense run of scientific investigation;
the pilot's stratified sample happened to undersample that density.
Documented in the ledger, not silently absorbed.

**Applied the Architect's standing rule** (new id only for a genuinely
different claim/mechanism, not a sharper/better-evidenced version of an
existing one) to all ~45 `NO-HYP` clusters, producing a disposition
table in the ledger: most fold into `HYP-P8`/`HYP-P13`/`HYP-P14`/`HYP-S1`
as dated updates (several are, on inspection, independent measurement
lineages converging on HYP-P14's own core claim — flagged explicitly,
since HYP-P14 itself doesn't currently cite them), a handful land as
genuinely new hypotheses, two are already sufficiently captured in
existing framing tables, two are superseded by their own later fixes.

**One finding needs prominent flagging, not just a citation**:
[[TASK-0216]]/[[TASK-0217]] show `search_complexity.md`'s own live
prose ("The one surviving positive: PTP1B `dcc_low` k=10...") no longer
holds — the positive collapses (AUC 1.000→0.598) once scored under
PTP1B's real, non-fallback active-site seed. This is a correction to an
existing claim, not a missing citation the checker would catch.

**Landing not applied.** The 2026-09-04 sign-off reviewed and ruled on
5 pilot drafts in detail; this sweep produced disposition
recommendations for ~45 clusters plus one flagged correction. Editing
`physics.md`/`search_complexity.md`/`reference_register.md` in ~20+
places is a bigger step than what was previewed — reporting the full
disposition table for a go-ahead before applying it, rather than
extending the existing authorization by inference.

## Done

**2026-09-04, Implementer A.** Landing applied after "resume please."
Full disposition table (`.claude/hypotheses/TASK_CLASSIFICATION_LEDGER.md`)
applied to `physics.md`/`search_complexity.md`: `HYP-P8`/`HYP-P9`/
`HYP-P13`/`HYP-P14`/`HYP-S1` extended with dated fold-in updates; ten
new hypotheses landed (`HYP-P15`-`HYP-P24`); one existing claim
corrected (`search_complexity.md`'s "one surviving positive," struck
through per the file's own no-silent-overwrite convention, replaced
with [[TASK-0216]]/[[TASK-0217]]'s finding that it was a
seed-construction artifact); one previously-open cross-reference
question in `search_complexity.md` (the HYP-P13 complexity conjunction's
"thermodynamically accessible" leg) closed directly by the
TASK-0227/0228/0230/0234/0235 cluster rather than becoming a new id.
No `reference_register.md` rows added — nothing in scope fit that
scheme. `.claude/hypotheses/INDEX.md` regenerated;
[[TASK-0322]]'s checker re-run clean (18/18 tests pass; remaining
staleness findings are self-citations from meta/process tasks about
the register itself, a known checker limitation, not new science).

**Final tally**: 344/344 non-meta tasks classified (`ENG` 204/59%,
`HYP-<id>` 58/17% pre-sweep + landed fold-ins, `NO-HYP` 82/24% across
~45 clusters, `UNCLASSIFIED` 0). Full ledger, disposition table, and
landing record: `.claude/hypotheses/TASK_CLASSIFICATION_LEDGER.md`.
