...
| TASK-0066 | Shared Kirchhoff-context + DCC numpy helper — dedupes `backend/analysis.py::gnm_context`/`_dcc` vs `allostery/potentials.py::_gnm_msf`/`V_R`/`V_C`/`V_M`'s independently re-derived pseudo-inverse math (TASK-0030-style port, not import); filed by TASK-0018's decision doc | Implementer | Done | P2 | 2026-07-12 | Implementer A (this thread) | 2026-07-12 20:42 | `.ai/tasks/DONE/TASK-0066-shared-kirchhoff-dcc-helper.md` |
...
| TASK-0068 | NISQ noise-model simulation (depolarizing + amplitude-damping Trotterized sim, top-5 degradation vs depth/error) consuming `coarse.py`'s output — owns SEAM-0010, filed per TASK-0013's own Open Question once `coarse.py` landed | Implementer | Done | P2 | 2026-07-14 | Implementer A (this thread) | 2026-07-14 18:39 | `.ai/tasks/DONE/TASK-0068-nisq-noise-model-simulation.md` |
...
| TASK-0074 | Characterization tests for `backend/analysis.py` (`gnm_context`/`site_potentials`/`quantum_seed_readiness`/`connectivity_change`) pinning current live output before any convergence touches it — hard-rule prerequisite for TASK-0066/0072 per `EXECUTION_PLAN.md` Phase 4.2 | Implementer | Done | P1 | 2026-07-12 | Implementer A (this thread) | 2026-07-12 21:42 | `.ai/tasks/DONE/TASK-0074-backend-analysis-characterization-tests.md` |
...
| TASK-0101 | Operator sweep Tier-1 harness — `analysis.operator_sweep` + `scripts/sweep_operators.py`, scores all 14+2 operators **× both propagators** (`time_averaged_ctqw`, `ground_state_relaxation`) against TASK-0094's proximity floor on all 3 mandatory targets (96 cells), reports a table. **Amended 2026-07-13**: must support chunked/resumable/parallel runs (`--target`/`--operator`/`--propagator` filters, per-cell result files, skip-if-exists + `--force` to recompute specific suspicious cells, `--aggregate` to merge partial runs from multiple machines). Tier-2 selection explicitly out of scope — follow-up from TASK-0100's decision | Implementer | TODO | P1 | 2026-07-13 | — | — | `.ai/tasks/TODO/TASK-0101-operator-sweep-tier1-harness.md` |
...
| TASK-0103 | Dumbbell 2x2 negative-control test suite (well vs. coupling confound, synthetic, no PDB/network) — GSR follows the well, CTQW follows the coupling; from REVIEW-2026-07-13b T-A, "do immediately, unblocks everything" | Implementer | TODO | P0 | 2026-07-13 | — | — | `.ai/tasks/TODO/TASK-0103-dumbbell-negative-control-suite.md` |
...
| TASK-0105 | ENAQT gamma-sweep on real mandatory targets — interior-optimum transport measurement (not AUC recovery), reframes the killed dephasing question; from REVIEW-2026-07-13b T-C, "highest scientific upside in the repo" | Implementer | Done | P0 | 2026-07-14 | Implementer A (this thread) | 2026-07-13 23:08 | `.ai/tasks/DONE/TASK-0105-enaqt-gamma-sweep-real-targets.md` |
| TASK-0106 | Reproduce the CTQW-trapping finding (REVIEW-2026-07-13c) on real BCR_ABL1 data — H10/H2 vs H_new(lambda=0.25) vs H_new(default) distal-pocket AUC + transport diagnostic, checked against the proximity floor | Implementer | Done | P0 | 2026-07-14 | Implementer A (this thread) | 2026-07-14 06:41 | `.ai/tasks/DONE/TASK-0106-ctqw-trapping-real-data-reproduction.md` |
