...
| TASK-0337 | `HYP-P25` (correction) | **Correction, not new** (2026-09-07) — TASK-0334's proximity-anticorrelation numbers were computed over structures, never referencing `pocket_distance.csv`'s own `cluster` column; the `is_distal` cohort is concentrated (`CAS0002` alone = 20%+ of it), so the reported n was inflated. Cluster-collapsed (median per cluster) + cluster-permutation p + cluster-bootstrap 95% CI: pipeline hit-rate vs. distance survives at 7/8 pre-registered combinations (cluster rho −0.34 to −0.50, n_clusters 22-55, permutation p 0.004-0.038); one combination (round 2/held-out/median_euclid, n=22, smallest+weakest metric) is borderline (p=0.058, CI crosses zero). PASSer control unweakened; random-arm control weakens from significant to non-significant (softened, not reversed). HYP-P25's Status updated with a dated correction, not overwritten. |
| TASK-0345 | `HYP-P26` (new) | **Landed directly** (2026-09-08) — leading cryptic-pocket benchmarks score on ligand-removed holo structures, not genuine apo; measured the (2)−(1) delta directly for the first time (63 validated apo/holo pairs, `cryptosite_pairs.json` + `pocketminer_pairs.json`, CryptoBench not fetched — vendored nowhere in this repo, stated scope reduction from the filing's assumed ≥100/1042). Result: computationally stripping a holo structure's ligand scores ~0.2 druggability points higher and ranks ~1 position better than a genuine apo structure of the same protein (mean delta +0.199, bootstrap 95% CI [+0.102,+0.296], sign-flip permutation p<1e-4). Positive control holds in aggregate and 76% of individual pairs. Genuinely distinct external-benchmark-validity mechanism from HYP-P21 (labeling defect) and HYP-P8/P9 (internal confounds) — new id, not a fold-in. |
| TASK-0350 | `HYP-P7`, `HYP-P6` | **Folded, not new** (2026-09-09) — direct, matched test of the interference question this register never actually ran: identical `H_new`, identical seed, identical 108-structure ASBench cohort, coherent=True vs coherent=False, plus a matched classical-diffusion twin (the genuinely-PSD graph Laplacian `H_new`'s own graph term, resolving HYP-P7's own "provided H_new is PSD" caveat). Planned Validation passed (bit-exact TASK-0308 reproduction). Result: coherent−decoherent resid-AUC delta n=108, mean +0.0023, Wilcoxon p=0.919, cluster-p=0.834 — pre-registered "small and non-significant" prediction HOLDS decisively. Folded into HYP-P7. Separately, in the same run: 50/108 (46%) coherent and 41/108 (38%) decoherent structures flip resid-AUC sign between T=15 and the converged limit — generalizes TASK-0119's 3-target finding to the full cohort; folded into HYP-P6. A first version of the script redid eigh(H) 5x per structure and ran 10+ hours without finishing — killed and fixed (shared eigh, flush=True progress logging) before the real run. |
| TASK-0361 | `HYP-P28` | **Folded, not new** (2026-09-09) — audit, not a finding: classifies every AUC-type number in `PHASE1_SUBMISSION_V3.md` as per-structure-averaged (immune to HYP-P28's pooled-protein-identity-channel confound) vs. pooled, `file:line` cited, code read not re-run. Planned Validation passed (correctly separated TASK-0310's known-per-structure 0.5921 from TASK-0359's known-pooled 0.6500 first). Result: every AUC-type number traces to a per-structure/per-cluster `roc_auc_score` call — none pooled; several other quoted numbers are count/rate statistics the mechanism doesn't apply to, marked N/A not silently skipped. HYP-P28's own pooled `protein_baseline_auc` finding is confirmed not currently quoted anywhere in the submission. Pre-registered "the submission is clean" prediction HOLDS. |
| TASK-0372 | `HYP-P29` (new) | **Landed directly** (2026-09-12) — external reviewer's ASBench aromatic-enrichment claim (15.7% vs ~10.2%, 1.54x) tested against a control the reviewer's own burial-only gate misses: ASBench's own ligand-contact labeling (40/40 sampled structures carry a bound ligand, TASK-0329/TASK-0345). Planned Validation: raw number reproduces almost exactly (15.6% vs 15.7%). Matched control (active sites, same protein) already gives ratio 1.22 not 1.54 -- a wrong-baseline sub-finding on its own. Burial-residualised, paired within-protein, cluster-tested by protein (79 clusters): not significant in any stratum (all p=0.856, contacting p=0.515, non-contacting p=0.406, sign reverses in the non-contacting stratum but n=20). Pre-registered most-likely outcome (burial+ligand-contact, effect vanishes) HOLDS -- a clean negative, distinct claim from every existing hypothesis, new id not a fold-in. |
| TASK-0374 (E3) | `ENG` | Not a scientific finding -- a computational feasibility/infrastructure result. Attempted the electronic-coupling (`ΔH_AB`) calculation E3 scoped: `pyscf`/`qiskit-addon-sqd` installed, real geometry verified (both 1W0E/1W0F are 5-coordinate at Fe, correcting the scoping's own 6-vs-5-coordinate assumption), fragments built from real coordinates. SCF did not converge under ROHF/UHF/UKS-B3LYP (~61 min total, 3 methods, all standard remedies). `qiskit_addon_sqd` cannot even import in this repo's x86_64-under-Rosetta `.venv` (jaxlib needs AVX, Rosetta doesn't emulate it) -- same root cause diagnosed and fixed for TASK-0372's own claim.py crash. `ΔH_AB` not computed; concrete next steps (native arm64 venv, larger basis, scf.newton()) recorded for whoever picks this up next. |
| TASK-0338 | `HYP-P13` (correction) + `ENG` | **Part A — correction, not new** (2026-09-07) — TASK-0336's own "4/276 vs 5/276" table had no variance/CI/p-value anywhere. Added exact Poisson 95% CIs (heavily overlapping: [1.09,10.24] vs [1.62,11.67]) and a paired exact test (McNemar, on the same 276 families both arms score — the correct test since it's one paired comparison, not two independent samples: 1 discordant family, p=1.0). HYP-P13's own Status updated with a dated correction: the honest reading is not "classical beats CTQW", it is "under matched multiplicity, no arm clears more than 5/276 and the arms are statistically indistinguishable". **Part B — `ENG`**, no hypothesis change: TASK-0318's own committed `no_proximity_feature_check.json` (the 0.6017 number) had no script producing it; added `--exclude-proximity` to `task0318_input_space_ceiling.py` and reproduced the committed JSON byte-for-byte from the existing feature cache. Confirms the number's provenance; does not change its claim (TASK-0318 itself is a **recommendation-only** `NO-HYP` candidate, per row above — not landed as a hypothesis by this or any prior task). |
| TASK-0348 | `HYP-P13` (sharpened, not new) | **Landed as a dated Status update** (2026-09-08) — the mandatory ablation against Mohtashim/Sajjan/Kais (JACS 2026, DOI 10.1021/jacs.6c08053), whose CTQW-on-protein-contact-graph construction is essentially this project's own and reports rho≈0.95 vs eigenvector centrality with no quantum advantage. Ran on TASK-0318's own 105-structure/74-protein cohort with `eigenvector_centrality`/`closeness_centrality` added to `baselines.py`; Planned Validation passed 105/105 exact-match. Found a genuine divergence, not smoothed over: median rho=0.41 vs eigenvector centrality (not ≈0.95), and AUC is not uniformly null — CTQW beats degree/GNM-alone/eigenvector centrality (p<0.05) but ties betweenness/closeness (p=0.50/0.93). Not a contradiction of HYP-P13: CTQW beats exactly the seed-BLIND arms and ties the two path/distance-based ones, consistent with the register's own "seed-referencing = proximity detector" mechanism, sharpened rather than reopened. Corrected [[PHASE1_SUBMISSION_V2.md]]'s own draft sentence, which had claimed "our measurements are consistent with" the published rho — they are not, at the specific-number level; the qualitative "no clean quantum advantage" reading survives. |
