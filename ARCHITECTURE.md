...
- 2026-06-28 · Oussema · 3D contact-graph animation: **real-structures mode**. New `analysis.morph_frames` + `GET /api/morph-frames` return Kabsch-aligned Cα keyframes `[apo, …user intermediates, holo]` on the residue set shared by all. The graph morph generalized from a single apo→holo straight line to a path through K real keyframes; a "Motion" toggle (straight-line ↔ real structures) + intermediate-PDB-id input added under the graph. Linear mode unchanged.
