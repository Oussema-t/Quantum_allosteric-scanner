...
- 2026-07-26 · Oussema · **Phase 2 slice 2 (§5f/§3) — allosteric-site PREDICTION** (the core deliverable). `quantum.py`: `allosteric_transfer` + `predict_allosteric_sites` — CTQW seeded at the active site, two modes: **occupation** (average-mixing matrix; pocket residues) and **pathway** (γ-damped Green's function `_green`; driver residues), on the documented disorder-suppressed winner operator (`H10_disorder_supp`, **un-tuned** mu=1.0), spatially de-duplicated top-5. `GET /api/allosteric?mode=occupation|pathway` (cached) + a "② Quantum solving — allosteric prediction" sub-panel (mode selector, top-5 chips, P@5 badge, 3D overlay: pink = predicted, teal = seed). **Validation (P@5 vs holo drug pocket, 6 Å) is isolated to `pocket_pk` — the predictor sees ONLY apo coords + seed; verified no leakage.** Cross-target (un-tuned): KRAS occ/pathway **P@5 0.6** (matches the documented ~0.6–0.8; occ≈pathway because KRAS active/allosteric overlap), BCR-ABL1 pathway 0.6 > occ 0.4 (documented pattern), PTP1B 0.0 (un-tuned miss). Per-target Optuna tuning NOT applied.
- 2026-07-26 · Oussema · Trap panel: user-selectable **Hamiltonian family** (GNM / GNM_bfactor / H10_disorder_supp / terminal_suppressed / rigidity_laplacian / H10B_disorder_bfactor) **and detection method** (spectral IPR / dynamical time-avg return / classical degree) — `/api/traps` `family` + new `method` param; `graph_trap_scan` switches the primary trap set accordingly (IPR spectrum + baseline/gate stay operator diagnostics). Empirically validated they give **materially different** trap sets (KRAS: families 0–34 traps, low overlap; spectral vs dynamical 0/10 overlap), so the choice is a real, labelled modelling decision — not cosmetic.
- 2026-07-26 · Oussema · Phase 2 slice 1 **§2c-viz** — interactive 3D trap viewer in the "② Quantum" panel: cutoff buttons (6/7/8/10/12/14 Å) re-run `/api/traps` at that contact cutoff and overlay the traps on a dedicated 3Dmol viewer (non-peripheral = red spheres, peripheral = grey, active-site seed = teal). Ported from the notebook py3Dmol + ipywidgets cutoff explorer; no new science. Verified `/api/traps` varies with cutoff (KRAS 23→10→6 traps at 6/8/14 Å; near-degen 13→25→75%).
- 2026-07-26 · Oussema · **Phase 2 slice 1 (notebook §2c)** — new `backend/quantum.py`: graph-trap pre-flight diagnostic. Ports `build_hamiltonian` (GNM = normalized Laplacian of the weighted contact graph) + `spectral_filter` (ported; **verified inert for trap detection** — reshapes eigenvalues, not eigenvectors) + `find_ctqw_traps` (localized dead-end modes: IPR / support / boundary-confinement) + degree baseline + degree-matched dynamical gate + threshold-stability + cutoff sweep, **seed-coupled to the active site** (which traps the active-site walk actually feeds). `GET /api/traps` (cached) + a "② Quantum — graph traps" panel (Plotly trap-strength bar + IPR localization spectrum). Citations **corrected after an adversarial multi-agent fact-check** (Bell&Dean 1970 / Wegner 1980 for IPR — the pasted "Thouless J.Phys.C 7 (1974)" was mis-cited; scope relabelled "finite-size localization", not a true Anderson transition). Validated on KRAS (169 res, 0.22 s) + BCR-ABL1 (451 res, 2 s); reported honestly as a **classical connectivity diagnostic** (degree reproduces 5–6/10 traps) that is **thin for BCR-ABL1** (63% near-degenerate states).
- 2026-07-24 · Oussema · Perf: pre-warm now **yields to live traffic**. A live-request counter (middleware, excl. `/api/health`) + `_wait_for_idle()` make the background pre-warm pause before every heavy item while any user request is in flight — on Render's 0.1-vCPU free tier the ungated pre-warm was starving clicks (uniform ~100 s even for a plain load; intrinsic load is only ~1.8 s benchmark / ~3.6 s auto locally). Also: the network-heavy real-structures **morph pre-warm is now OFF by default** (`QAS_PREWARM_MORPH=0`). Note: pre-warm covers benchmark active-site mode; an "auto (UniProt)" load is a cache miss and recomputes.
- 2026-07-24 · Oussema · Perf (no scientific change): `seed_readiness_shift` (run inside `/api/connectivity-change`) was ~46 s because the 2σ bootstrap noise-floor recomputed the full O(N³) GNM covariance on every one of ~800 resamples. Now the raw per-residue arrays (msf / |nDCC| coupling / slow-mode) are precomputed ONCE per structure (`_raw_arrays`) and reused across the active-site + pocket shifts. Verified byte-identical output; 45.4 s → 4.1 s locally (KRAS), so the uncached connectivity panel drops from ~458 s → ~40–60 s on Render (cached/pre-warmed hits stay instant).
- 2026-07-04 · Oussema · UI: every result section now shows its compute time (⏱ seconds waited) with a ⚡cached marker when served from the result cache — load/GNM, structure intel, holo finder, compare, shift, connectivity change, real-structures morph.
- 2026-07-04 · Oussema · Cache everything: the param-keyed result cache now wraps ALL heavy endpoints — `/api/load`, `/api/connectivity-change`, `/api/analysis-shift`, `/api/morph-frames`, `/api/compare`, `/api/holo-finder`, `/api/active-site`, `/api/drug-site`, `/api/structure` (add-only `cached` flag; helper `_cache_or_compute`). Pre-warm extended to also warm connectivity + apo→holo shift + compare + the real-structures morph (`QAS_PREWARM_MORPH*`) for every benchmark target. Nothing truncated.
- 2026-07-04 · Oussema · Startup pre-warm: a daemon thread loads the 6 challenge targets (apo load + connectivity change) into the result cache after boot (`QAS_PREWARM*` env vars), so the first jury click on a benchmark is instant (cache hit ~0.03 ms vs ~900 ms compute). Also caches every benchmark structure, so any later cutoff is a network-free sub-second recompute. Nothing truncated — full proteins, dense eigh.
- 2026-07-04 · Oussema · Performance pass (no scientific change): new `config.py` (all tunables → env vars) + `result_cache.py` (SHA-256 param-keyed in-memory LRU + disk cache) wired into `/api/load` and `/api/connectivity-change` (add-only `cached` flag); `/healthz` alias + cache stats on `/api/health`. Kept dense GNM `eigh` — `scripts/verify_eigensolver.py` proves a truncated sparse solve shifts the shipped MSF ~45% (not equivalence-preserving). Job queue / progress bar deferred. See PROFILING.md + CHANGES.md.
- 2026-07-04 · Oussema · Morph intermediates: quality gates from a structural-biologist review (colleague). `_load_best_chain` now picks the chain that actually matches our protein (max residue overlap) — fixes antibody-complex structures like KRAS 3GFT where "chain A" was the antibody. Candidates are rejected if overlap <80% or Cα-RMSD-to-apo >10 Å, auto-dropping mis-aligned/wrong-construct outliers (e.g. 1KZP at 19 Å) with no hardcoded PDB list; `rejected[]` returned + shown in the UI. Validated on KRAS: 1KZP dropped, 3GFT→real KRAS chain, others kept.
